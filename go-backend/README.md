...
- Catalog data: written by [`stockroom_crawler/`](../stockroom_crawler/), not by this service.
...
```bash
make api                          # from the repo root: Postgres + MinIO + api
...
`MINIO_*`). Populate the catalog with [`stockroom_crawler/`](../stockroom_crawler/) first — an empty

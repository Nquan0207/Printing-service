    ...
    try {
      const out = readResult<{ status?: string }>(
        await app.callServerTool({
          ...
          // put on the wire.
          // The backend matches name AND email against the ops administrator,
          // so both travel; the passcode only when this server checks one.
          arguments: passcodeRequired
            ? { name: name.trim(), email: email.trim(), passcode }
      ...
      <Stack gap="xs">
        ...
        <Group justify="flex-end">
          <Button
            size="xs"
            loading={busy}
            disabled={!name.trim() || !email.trim() || (passcodeRequired && !passcode)}
            onClick={submit}
          >
            Sign in
          </Button>
